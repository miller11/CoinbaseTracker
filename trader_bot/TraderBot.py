...
class TraderBot:
        ...
        if self.last_transaction is not None:
            if self.last_transaction['operation'] == 'BUY':
                ...
                if investment_util.should_sell(buy_amount=float(self.last_transaction['amount']),
                    ...
                    self.__record_decision(investment_util.get_decision(), investment_util.get_reason(),
                                           investment_util.get_margin())
                    self.__put_transaction(investment_util.get_decision(), self.account['native_balance']['amount'])
                ...
                if investment_util.should_buy():
                    self.__record_decision(investment_util.get_decision(), investment_util.get_reason())
                    self.__put_transaction(investment_util.get_decision(),
    ...
    def __put_transaction(self, operation, amount):
        table = get_dynamo_table('cb-bot-transaction')
        transaction = {
        ...
        if operation == 'SELL':
            transaction['currency_value'] = self.__get_sell_price()
        else:
            transaction['currency_value'] = self.__get_buy_price()
    ...
    def __get_account(self):
        client = self.__get_cb_client()

        return client.get_account(account_id=self.config.account_id)

    def __get_buy_price(self):
        client = self.__get_cb_client()

        return client.get_buy_price(currency_pair='BTC-USD').amount

    def __get_sell_price(self):
        client = self.__get_cb_client()

        return client.get_sell_price(currency_pair='BTC-USD').amount

    def __record_decision(self, decision, reason, margin=None):
